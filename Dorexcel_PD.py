...
class ConcatenarExcelGUI:
    def __init__(self):
        self.window = tk.Tk()
        self.window.title("Dorexcel_PD")
        ...
        self.label_arquivo1 = tk.Label(self.window, text="Arquivo 1: Excel Clientes > aba Customers")
        self.label_arquivo1.pack()

        self.btn_arquivo1 = tk.Button(self.window, text="Selecionar", command=self.selecionar_arquivo1)
        self.btn_arquivo1.pack(pady=5)

        self.label_arquivo2 = tk.Label(self.window, text="Arquivo 2: Excel Clientes > aba Addresses")
        self.label_arquivo2.pack()

        self.btn_arquivo2 = tk.Button(self.window, text="Selecionar", command=self.selecionar_arquivo2)
        self.btn_arquivo2.pack(pady=5)

        self.label_arquivo3 = tk.Label(self.window, text="Arquivo 3: Excel Vendas > aba Worksheet")
        self.label_arquivo3.pack()

        self.btn_arquivo3 = tk.Button(self.window, text="Selecionar", command=self.selecionar_arquivo3)
        self.btn_arquivo3.pack(pady=5)

        self.btn_concatenar = tk.Button(self.window, text="Gerar nova planilha", command=self.concatenar_excel)
        self.btn_concatenar.pack(pady=20)

    def run(self):
        self.window.mainloop()

    def selecionar_arquivo1(self):
        self.arquivo1 = filedialog.askopenfilename(filetypes=[("Arquivo Excel", "*.xlsx")])
        self.label_arquivo1.configure(text="Arquivo 1: Excel Clientes > aba Customers" + self.arquivo1)

    def selecionar_arquivo2(self):
        self.arquivo2 = filedialog.askopenfilename(filetypes=[("Arquivo Excel", "*.xlsx")])
        self.label_arquivo2.configure(text="Arquivo 2: Excel Clientes > aba Addresses" + self.arquivo2)

    def selecionar_arquivo3(self):
        self.arquivo3 = filedialog.askopenfilename(filetypes=[("Arquivo Excel", "*.xls")])
        self.label_arquivo3.configure(text="Arquivo 3: Excel Vendas > aba Worksheet" + self.arquivo3)

    def concatenar_excel(self):
        if self.arquivo1 == "" or self.arquivo2 == ""or self.arquivo3 == "":
            tk.messagebox.showerror("Erro", "Selecione os 3 arquivos necessários.")
            return

        try:
            # Ler os arquivos .xlsx
            Arq1 = pd.read_excel(self.arquivo1, sheet_name='Customers')
            Arq2 = pd.read_excel(self.arquivo2, sheet_name='Addresses')
            Arq3 = xlrd.open_workbook(self.arquivo3)
            ...
            tk.messagebox.showinfo("Conclusão", "A nova planilha foi gerada com sucesso!")
        except Exception as e:
            tk.messagebox.showerror("Erro", str(e))


# Instanciar a classe ConcatenarExcelGUI e executar a interface gráfica
gui = ConcatenarExcelGUI()
gui.run()
